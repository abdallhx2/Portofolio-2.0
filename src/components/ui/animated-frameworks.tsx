...
const ClaudeIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg className={className} style={style} viewBox="0 0 1200 1200" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
const FRAMEWORK_PATH = "M 1 0 v 5 q 0 5 5 5 h 39 q 5 0 5 5 v 71 q 0 5 5 5 h 39 q 5 0 5 5 v 5"
...
const AnimatedFrameworks = () => {
  ...
  // Card cycling animation — only runs after visible
  useEffect(() => {
    if (!isVisible) return
  ...
  const cardClasses =
    "flex aspect-square items-center justify-center rounded-md border p-1.5 [@media(min-width:500px)]:p-3 lg:p-4 " +
    "shadow-[0_8px_30px_rgb(0,0,0,0.12)] " +
    "h-12 [@media(min-width:500px)]:h-20 lg:h-36 " +
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "h-[7rem] sm:h-[10rem] lg:h-[19rem] w-full"
      )}
    >
      {/* SVG S-curve path with animated beam */}
      <div className="absolute flex h-full w-full items-center justify-center">
        <div className="h-full w-[11rem] sm:w-[12rem] lg:w-[15rem]">
          <svg
          ...
          >
            ...
            >
              <path d={FRAMEWORK_PATH} />
            </g>

            {/* Animated beam — only runs when visible */}
            {isVisible && (
      ...
      >
        ...
        >
          <ClaudeIcon className="size-5 [@media(min-width:500px)]:size-7 lg:size-9" style={{ color: 'var(--foreground)' }} />
        </div>
        <div
          className={cardClasses}
          style={{
            transform: nextJsTransform,
            background: 'linear-gradient(to bottom, var(--background), var(--card))',
            borderColor: 'var(--border)',
          }}
        >
          <SiGithub className="size-5 [@media(min-width:500px)]:size-7 lg:size-9" style={{ color: 'var(--primary)' }} />
        </div>
        <div
          className={cardClasses}
          style={{
        ...
        >
          <SiOpenai className="size-5 [@media(min-width:500px)]:size-7 lg:size-9" style={{ color: 'var(--foreground)' }} />
        </div>
      </div>
