...
export default function Footer() {
  ...
  return (
    <footer className="border-t py-8 section-padding" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border)' }}>
      <div className="max-w-[var(--container-max)] mx-auto">
        <div className={`flex flex-col md:flex-row justify-between items-center gap-6 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
          {/* Copyright */}
          <div className={`text-center ${isRTL ? 'md:text-right' : 'md:text-left'}`}>
            <p className="text-body font-medium" style={{ color: 'var(--foreground)' }}>
              {t('footer.madeBy') || 'Made by'} <span className="font-semibold" style={{ color: 'var(--primary)' }}>Abdullah</span>
            </p>
            <p className="text-body-sm mt-1" style={{ color: 'var(--muted)' }}>
